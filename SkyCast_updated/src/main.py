...
from advanced_functions import process_radar_with_advanced, WeatherAlert
...
async def get_advanced_radar_data(
    district: str = Query("ЦФО", description="Федеральный округ"),
    enable_nowcast: bool = Query(True, description="Включить nowcasting"),
    enable_alerts: bool = Query(True, description="Включить предупреждения"),
):
    """
    Расширенный API с nowcasting, статистикой, ячейками и предупреждениями.
    
    Returns:
        Полный набор данных: smoothed_field, forecasts, cells, alerts, statistics
    """
    if district not in FEDERAL_DISTRICTS_BBOX:
        raise HTTPException(status_code=400, detail=f"Неизвестный округ: {district}")
    ...
    if data is None:
        raise HTTPException(status_code=503, detail="Нет данных для обработки")
    
    try:
        # Запускаем расширенную обработку
        result = process_radar_with_advanced(
            data["grid_lon"],
            data["grid_lat"],
            data["z"],
            district,
            enable_nowcast=enable_nowcast,
            enable_alerts=enable_alerts,
            smooth_sigma=0.5,
        )
        
        # Добавляем базовую информацию
        result["stations_count"] = data["stations_count"]
        result["timestamp"] = cache.timestamp.isoformat() if cache.timestamp else datetime.utcnow().isoformat()
        
        # Преобразуем numpy массивы в списки для JSON
        if "smoothed_field" in result:
            result["smoothed_field"] = result["smoothed_field"].tolist()
        if "filled_field" in result:
            result["filled_field"] = result["filled_field"].tolist()
        
        # Forecasts тоже конвертируем
        if "forecasts" in result:
            for key in result["forecasts"]:
                if "field" in result["forecasts"][key]:
                    result["forecasts"][key]["field"] = result["forecasts"][key]["field"].tolist()
        
        return JSONResponse(content=result)
        
    except Exception as exc:
        log.error("Ошибка расширенной обработки: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail=str(exc))
